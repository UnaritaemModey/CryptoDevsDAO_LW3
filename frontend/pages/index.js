...
export default function Home() {
  ...
  async function createProposal() {
    ...
    try {
     ...
     setTimeout(async() => await waitForTransaction(tx), 3000)
    } catch (error) {
      console.error(error);
      window.alert(error);
    }
    ...
    <div className={inter.className}>
      ...
      <div className={styles.main}>
        <div>
          ...
          <div className={styles.description}>
            Your CryptoDevs NFT Balance: {nftBalanceOfUser?.toString()}
            <br />
            {daoBalance.data && (
              <>
                Treasury Balance:{" "}
            ...
            <br />
            Total Number of Proposals: {numOfProposalsInDAO ?.toString()}
          </div>
          <div className={styles.flex}>
            <button
